(() => {
  ...
  const canvas = document.querySelector("[data-fire-canvas]");
  ...
  const clipSections = [...document.querySelectorAll("[data-clip-section]")];
  ...
  const lastClipTime = new WeakMap();
  let loadedCount = 0;
  let currentFrame = 0;
  let canvasWidth = 0;
  let canvasHeight = 0;
  ...
  function drawFrame(index) {
    ...
    if (!image) return;
    // cover-fit: the sear fills the frame edge to edge
  ...
  function updateClip(section, progress) {
    // drift + zoom feed both the video and the poster/gradient fallback (slow parallax)
    section.style.setProperty("--clip-drift", ((progress - 0.5) * -6).toFixed(2));
    section.style.setProperty("--clip-zoom", progress.toFixed(3));
    const video = section.querySelector("[data-clip]");
    if (!section.classList.contains("has-clip") || !video) return;
    if (Number.isFinite(video.duration) && video.duration > 0) {
      const target = clamp(progress, 0, 0.997) * video.duration;
      const last = lastClipTime.get(video) ?? -1;
      if (Math.abs(target - last) > 0.025) {
        video.currentTime = target;
        lastClipTime.set(video, target);
        video.dataset.time = target.toFixed(2);
      }
    }
  }

  function armClips() {
    // ponytail: fetch->blob keeps scrubbing frame-exact and lets a missing clip fail
    // silently — r.ok=false leaves the styled fallback art in place, no error thrown
    clipSections.forEach((section) => {
      ...
      fetch(video.dataset.src)
        ...
        .then((blob) => {
          ...
          section.classList.add("has-clip");
        })
        .catch(() => {});
    });
  }
  ...
  function updateScrollState() {
    ...
    const heroFrame = reducedMotion ? 71 : Math.round(heroProgress * (FRAME_COUNT - 1));
    ...
    stickyChapters.forEach((section) => {
      ...
      if (section.hasAttribute("data-clip-section")) updateClip(section, progress);
    });
  ...
  function setupForm() {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (invalid) {
        ...
        invalid.focus();
        status.textContent = "Please complete your reservation details.";
        return;
      }
      form.dataset.submitted = "true";
      form.querySelector("button span").textContent = "Table requested";
      status.textContent = "Your table by the fire is requested. We will confirm by email.";
